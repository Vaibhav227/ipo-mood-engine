...
import {
  calculateMarketMeters,
  type MarketMeterKey,
  type MarketMeters
} from "../../../../packages/scoring/src/marketMeters.js";
import type { MoodCategory } from "../../../../packages/scoring/src/moodTaxonomy.js";

export type MoodSnapshotDto = {
  slug: string;
  name: string;
  snapshotDate: string;
...
function withMarketMeters<T extends { moodScores: Record<string, number> }>(item: T): T & { marketMeters: MarketMeters } {
  return {
    ...item,
    marketMeters: calculateMarketMeters(item.moodScores as Record<MoodCategory, number>)
  };
}

function addMarketMeters(items: Omit<MoodSnapshotDto, "marketMeters">[]): MoodSnapshotDto[] {
  return items.map((item) => withMarketMeters(item));
}

export async function getLatestMoodSnapshots() {
  const items =
    (await psqlJson<Array<Omit<MoodSnapshotDto, "marketMeters">> | null>(`
...
export async function getLatestMoodBySlug(slug: string) {
  const item = await psqlJson<Omit<MoodSnapshotDto, "marketMeters"> | null>(`
...
export async function getMarketMood(): Promise<MarketMoodDto> {
  const latest = await getLatestMoodSnapshots();
  const moodCounts = new Map<string, number>();
  const scoreTotals = new Map<string, number>();
  const meterTotals = new Map<MarketMeterKey, number>();
  ...
  for (const item of latest.items) {
    ...
    for (const [key, meter] of Object.entries(item.marketMeters) as Array<[MarketMeterKey, MarketMeters[MarketMeterKey]]>) {
  ...
  ) as Record<MarketMeterKey, number>;
  const leaders = {
