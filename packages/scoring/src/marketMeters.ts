import type { MoodCategory } from "./moodTaxonomy.js";
...
export type MarketMeter = {
  ...
  verdict: string;
  reason: string;
  positiveDrivers: Array<{ key: MoodCategory; label: string; value: number }>;
  riskDrivers: Array<{ key: MoodCategory; label: string; value: number }>;
};

export type MarketMeters = Record<MarketMeterKey, MarketMeter>;

const scoreLabels: Record<MoodCategory, string> = {
  fomo_frenzy: "FOMO",
...
const bandForScore = (score: number): MarketMeter["band"] => {
...
const topDrivers = (
  ...
  weights: Partial<Record<MoodCategory, number>>,
  minContribution = 2
) =>
  Object.entries(weights)
    .map(([key, weight]) => {
      const moodKey = key as MoodCategory;
...
const weightedScore = (
  scores: Record<MoodCategory, number>,
  positives: Partial<Record<MoodCategory, number>>,
  risks: Partial<Record<MoodCategory, number>>
) => {
  const positiveScore = Object.entries(positives).reduce(
    (sum, [key, weight]) => sum + (scores[key as MoodCategory] ?? 0) * (weight ?? 0),
    0
  );
...
const driverText = (drivers: Array<{ label: string; value: number }>) => {
  if (drivers.length === 0) return "no strong positive driver";
  return drivers.map((driver) => `${driver.label} ${driver.value}`).join(", ");
};

const riskText = (drivers: Array<{ label: string; value: number }>) => {
  if (drivers.length === 0) return "no major risk driver";
  return drivers.map((driver) => `${driver.label} ${driver.value}`).join(", ");
};

export function calculateMarketMeters(scores: Record<MoodCategory, number>): MarketMeters {
  const listingPositives: Partial<Record<MoodCategory, number>> = {
  ...
  const listingRisks: Partial<Record<MoodCategory, number>> = {
    operator_hype: 0.22,
    panic: 0.2,
    low_conviction: 0.18,
  ...
  const longTermPositives: Partial<Record<MoodCategory, number>> = {
  ...
  const longTermRisks: Partial<Record<MoodCategory, number>> = {
    operator_hype: 0.22,
    blind_hype: 0.2,
    panic: 0.18,
    low_conviction: 0.16,
